...
def number_game_likelihood(hypothesis, data):
    ...
    # Check if data is consistent with the given hypothesis
    data_b = np.array(data, dtype=bool)
    hypothesis_b = np.array(hypothesis, dtype=bool)
    if np.any(data_b & ~hypothesis_b):
        return np.log(0)  # log(0) represents -inf, as P(D|H) = 0 if data is not consistent with hypothesis

    # Count the number of options in the hypothesis
    num_options = np.sum(hypothesis)
    
    # If hypothesis has no options, it is not consistent with any data
    if num_options == 0:
        return np.log(0)  # log(0) represents -inf
    
    # Compute the log likelihood
    log_likelihood = np.sum(data) * np.log(1.0 / num_options)
...
hypotheses, priors = number_game_simple_init(100, 0.55, 0.45)
print(priors)
